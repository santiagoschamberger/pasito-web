...
export const metadata: Metadata = {
  title: 'Tu entrada · Silver Walks by Nutren',
...
export default async function SilverTicketPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: order, error: orderError } = await db
    ...
    .maybeSingle()
  if (orderError || !order || order.event_slug !== SILVER_EVENT.slug) notFound()

  const requestHeaders = await headers()
  const host = requestHeaders.get('x-forwarded-host') || requestHeaders.get('host') || 'pasito.app'
  const protocol = requestHeaders.get('x-forwarded-proto') || (host.includes('localhost') ? 'http' : 'https')
  const ticketUrl = `${protocol}://${host}/silver/ticket/${token}`
  ...
  return (
    <main className={styles.page}>
      <Link href="/silver" className={styles.logo} aria-label="Volver al evento">
        <Image src="/brand/logo-green.svg" alt="Pasito" width={112} height={27} priority />
      </Link>

      <article className={styles.ticket}>
        <header>
          <p>Silver Walks by Nutren</p>
          <h1>Tu entrada</h1>
        ...
        <dl>
          <div><dt><Clock3 size={18} /> Fecha y hora</dt><dd>{SILVER_EVENT.dateLabel}<br />{SILVER_EVENT.timeLabel}</dd></div>
          <div><dt><MapPin size={18} /> Lugar</dt><dd>{SILVER_EVENT.venueLabel}</dd></div>
        </dl>

        <p className={styles.note}>Mostrá este QR al ingresar. Es único y puede validarse una sola vez. No hace falta imprimirlo.</p>
      </article>

      <Link href="/contacto" className={styles.recover}>Necesito ayuda con mis entradas</Link>
    </main>
  )
}
