...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      || metadata?.eventSlug !== SILVER_EVENT.slug
    ...
    if (emailPending && shouldSendEmail) {
      ...
      try {
        ...
        console.error('[silver/orders] No se pudo enviar o registrar el email:', errorMessage)
        ...
        if (trackingError) console.error('[silver/orders] No se pudo registrar el intento de email:', trackingError)
      }
    } else if (isE2e) {
      emailPending = false
    }
    ...
    console.error('[silver/orders] No se pudo confirmar la orden:', error)
