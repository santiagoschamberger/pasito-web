  ...
  useEffect(() => {
    ...
    if (detected === 'android') {
      window.location.replace(
        buildChallengeAndroidIntentUrl(challengeId, playStoreUrl),
      )
      return
    }
    ...
    if (detected === 'ios') {
      ...
      window.location.href = buildChallengeCustomSchemeUrl(challengeId)
      return
    }

    setStage('desktop')
  }, [challengeId, appStoreUrl, playStoreUrl])
    ...
    return (
      <div className="grid gap-4 text-center">
        <p className="text-sm" style={{ color: 'rgba(255,255,255,0.86)' }}>
          Abrí este link desde tu teléfono. Si ya tenés Pasito, actualizá la app
          si te lo pide.
        </p>
        <div
  ...
  return (
    <div className="grid place-items-center gap-4">
      ...
      {platform === 'ios' ? (
        ...
        >
          ¿No se abrió? Bajá o actualizá la app
        </a>
      ) : null}
    </div>
  )
