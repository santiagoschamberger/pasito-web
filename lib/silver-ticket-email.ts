...
import { SILVER_EVENT, silverMoney, type EventTicket } from '@/lib/silver-event'
...
function ticketUrl(origin: string, ticketId: string): string {
  return `${origin}/silver/ticket/${createTicketToken(ticketId)}`
}

export async function sendSilverTicketsEmail(params: {
  origin: string
  ...
  const intro = params.kind === 'confirmation'
    ? `${greeting}tu pago está confirmado. ${allTickets.length === 1 ? 'Esta es tu entrada' : 'Estas son tus entradas'} para ${escapeHtml(SILVER_EVENT.name)}.`
    : `${greeting}acá tenés nuevamente ${allTickets.length === 1 ? 'tu entrada' : 'tus entradas'} para ${escapeHtml(SILVER_EVENT.name)}.`
  ...
  <body style="margin:0;padding:0;background:#f3f5ed;">
    <div style="max-width:560px;margin:0 auto;padding:38px 22px 48px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#183327;">
      ...
      <div style="background:#fff;border-radius:0 0 24px 24px;padding:30px 24px;">
        <p style="margin:0 0 8px;color:#0c6b45;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;">Silver Walks by Nutren · Organizan Pasito + Kiwell en colaboración</p>
        <h1 style="margin:0 0 14px;font-size:27px;line-height:1.12;">${params.kind === 'confirmation' ? '¡Nos vemos en Augusta!' : 'Tus entradas, de nuevo'}</h1>
        ...
        <div style="margin:0 0 24px;padding:17px 18px;border-radius:14px;background:#effa7a;color:#17382a;">
          <p style="margin:0 0 5px;font-size:14px;"><strong>${escapeHtml(SILVER_EVENT.dateLabel)}</strong> · ${escapeHtml(SILVER_EVENT.timeLabel)}</p>
          <p style="margin:0;font-size:14px;">${escapeHtml(SILVER_EVENT.venueLabel)}</p>
          ${params.kind === 'confirmation' ? `<p style="margin:7px 0 0;font-size:14px;">Total: <strong>${silverMoney(totalAmount)}</strong></p>` : ''}
        </div>
        ${rewardCards}
        ${ticketCards}
